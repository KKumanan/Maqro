...
const AppPopup: React.FC<AppPopupProps> = ({ isOpen, onClose, app }) => {
  ...
  const handleSaveClick = (macroId: string) => {
    // Find which list the macro is in
    const isSuggested = suggestedMacros.some(macro => macro.id === macroId);
    const targetList = isSuggested ? suggestedMacros : savedMacros;
    
    // Update the macro in the appropriate list
    const updatedMacros = targetList.map(macro => {
      if (macro.id === macroId) {
        return {
          ...macro,
          title: editedTitle,
          keybind: editedKeybind,
    ...
    // Update the state
    if (isSuggested) {
      setSuggestedMacros(updatedMacros);
    } else {
  ...
  const handleRemoveMacro = (macroId: string, isSuggested: boolean) => {
    if (isSuggested) {
      setSuggestedMacros(prevMacros => prevMacros.filter(macro => macro.id !== macroId));
    } else {
      setSavedMacros(prevMacros => prevMacros.filter(macro => macro.id !== macroId));
    }
  };

  const handleApproveMacro = (macroId: string) => {
    const macroToApprove = suggestedMacros.find(macro => macro.id === macroId);
    if (macroToApprove) {
      // Remove from suggested
      setSuggestedMacros(prevMacros => prevMacros.filter(macro => macro.id !== macroId));
      // Add to saved with isApproved set to true
      setSavedMacros(prevMacros => [...prevMacros, { ...macroToApprove, isApproved: true }]);
    }
  };
          ...
          <div key={macro.id} className={`macro-item ${editingMacroId === macro.id ? 'editing' : ''}`}>
            ...
            <div className="macro-actions">
              {editingMacroId === macro.id ? (
                ...
                <>
                  <button className="action-button execute-button" onClick={() => handleExecuteMacro(macro.id)}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <button 
                    ...
                    onClick={() => handleRemoveMacro(macro.id, title === "Suggested Macros")}
                  >
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  {title === "Suggested Macros" && (
                    <button 
                      className={`action-button approve-button ${macro.isApproved ? 'approved' : ''}`}
